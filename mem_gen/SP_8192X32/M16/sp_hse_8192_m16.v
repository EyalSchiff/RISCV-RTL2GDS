...
//       
//       Creation Date:  Wed Jan  7 17:49:44 2026
//       Version: 	r1p0
//
...
`ifdef POWER_PINS
module sp_hse_8192_m16 (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS,
    GWEN, RET1N);
`else
module sp_hse_8192_m16 (Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, GWEN, RET1N);
`endif
...
`endif
 ...
 wire  CLK_;
  ...
  buf B109(D_[30], D[30]);
  buf B110(D_[31], D[31]);
  buf B111(EMA_[0], EMA[0]);
  buf B112(EMA_[1], EMA[1]);
  buf B113(EMA_[2], EMA[2]);
  buf B114(EMAW_[0], EMAW[0]);
  buf B115(EMAW_[1], EMAW[1]);
  buf B116(EMAS_, EMAS);
  buf B117(GWEN_, GWEN);
  buf B118(RET1N_, RET1N);

   `ifdef ARM_FAULT_MODELING
     sp_hse_8192_m16_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .GWEN(GWEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module sp_hse_8192_m16_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [31:0] Q_out;
   input [31:0] Q_in;
   input CLK;
   input [12:0] A;
